...
import { RoleActionEffect } from './effects/RoleActionEffect';
...
export function HunterShotPanel({ prompt, players }: HunterShotPanelProps) {
  ...
  function handleShoot(id: string) {
    if (submitted) return;
    socket.emit(ClientEvents.HUNTER_SHOOT, { targetId: id });
    setSubmitted(true);
    setTargetId(id);
  }
  ...
  if (targetId) {
    ...
    return (
      <RoleActionEffect
        kind="hunter"
        targets={[{ nickname: target?.nickname ?? 'Người chơi', avatarUrl: target?.avatarUrl }]}
        onDone={() => setTargetId(null)}
      />
    );
  }
