...
import { DayRevealPayload, GamePhase } from '@werewolf/shared';
...
import { SpectatorNightEffect } from '../components/game/effects/SpectatorNightEffect';
import { NightActionLogView } from '../components/game/effects/NightActionLogView';
...
import { NightKillOverlay } from '../components/game/NightKillOverlay';
...
export function RoomPage({ roomId, onLeaveRoom, onOpenCardGallery }: RoomPageProps) {
  ...
  const [nightKillQueue, setNightKillQueue] = useState<DayRevealPayload['deaths']>([]);
  const [nightKillIndex, setNightKillIndex] = useState(0);
  const [executingPlayerId, setExecutingPlayerId] = useState<string | null>(null);
  ...
  const lastVoteResultRef = useRef(lastVoteResult);
  ...
  // Bell on new day reveal, then — once the existing "TRỜI ĐÃ SÁNG" phase
  // transition banner (3.2s, see the transition effect above) has cleared
  // the screen — play the cinematic kill overlay for each night death in
  // sequence, one at a time, before the normal DayRevealBanner takes over.
  useEffect(() => {
    if (lastDayReveal && lastDayReveal !== lastDayRevealRef.current) {
      audioManager.playSfx('bell');
      lastDayRevealRef.current = lastDayReveal;
      if (lastDayReveal.deaths.length > 0) {
        const t = setTimeout(() => {
          setNightKillQueue(lastDayReveal.deaths);
          setNightKillIndex(0);
        }, 3200);
        return () => clearTimeout(t);
      }
  ...
  // Cinematic execution overlay whenever a day vote actually kills someone
  // (a spared Idiot, a Maid swap, or an unresolved tie all leave
  // eliminatedPlayerId null — no overlay in those cases).
  useEffect(() => {
    if (lastVoteResult && lastVoteResult !== lastVoteResultRef.current) {
      lastVoteResultRef.current = lastVoteResult;
      if (lastVoteResult.eliminatedPlayerId) {
        setExecutingPlayerId(lastVoteResult.eliminatedPlayerId);
      }
    }
  }, [lastVoteResult]);
  ...
  return (
    <div className={`rm-stage ${isNight ? 'is-night' : ''}`}>
      ...
      {roomState.phase === GamePhase.NIGHT && me && !me.isAlive && (
        <>
          <NightActionLogView entries={nightActionLog} players={roomState.players} />
          <SpectatorNightEffect nightActionConfirmed={nightActionConfirmed} players={roomState.players} />
        </>
      )}
      ...
      {nightKillIndex < nightKillQueue.length && (() => {
        const death = nightKillQueue[nightKillIndex];
        const victim = roomState.players.find((p) => p.id === death.playerId);
        return (
          <NightKillOverlay
            key={death.playerId}
            nickname={victim?.nickname ?? 'Người chơi'}
            avatarUrl={victim?.avatarUrl}
            onDone={() => setNightKillIndex((i) => i + 1)}
          />
        );
      })()}
      {executingPlayerId && (() => {
        const victim = roomState.players.find((p) => p.id === executingPlayerId);
        return (
          <ExecutionOverlay
            key={executingPlayerId}
            nickname={victim?.nickname ?? 'Người chơi'}
            avatarUrl={victim?.avatarUrl}
            onDone={() => setExecutingPlayerId(null)}
          />
        );
      })()}
