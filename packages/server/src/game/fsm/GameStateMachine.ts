...
export class GameStateMachine {
   ...
   *
   * Known limitation: this runs after resolveNight() has already logged
   * night history and run chief-succession detection for the original
   * (pre-swap) death list — a revived Village Chief or an already-cascaded
   * Cupid heartbreak death are not retroactively undone. Rare enough in
   * practice (requires that specific role landing on the swap) that it's
   * left as a known edge case rather than reworking the whole pipeline.
   */
  applyMaidNightSwap(
    maidId: string,
    ...
    return {
      deathDetails: [
        ...previous.deathDetails.filter((d) => d.playerId !== chosenVictimId),
        ...applied.deathDetails,
      ],
      hunterPendingShotIds: [
        ...previous.hunterPendingShotIds.filter((id) => id !== chosenVictimId),
        ...applied.hunterPendingShotIds,
      ],
      wildChildTurnedIds: [
        ...previous.wildChildTurnedIds.filter((id) => id !== chosenVictimId),
